...
export const generateCourseSummary = (formattedData: FormattedOutput): CourseSummary => {
    ...
    const totalContentItems = course.units.reduce(
        (sum, unit) => sum + unit.topics.reduce((topicSum, topic) => topicSum + topic.contents.length, 0), 
        0
    );
    
    // Get all content items
    const allContents = course.units.flatMap(unit => 
        unit.topics.flatMap(topic => topic.contents)
    );
    ...
    const contentTypes = {
        ...
        other: allContents.filter(content => 
            !["Video", "Exercise", "Article", "Topic quiz", "Topic unit test"].includes(content.contentKind)
        )
    };
    
    // Analyze video content
    const videosWithMetadata = contentTypes.videos.filter(video => video.videoMetadata);
    const videosWithKeyMoments = videosWithMetadata.filter(video => 
        video.videoMetadata?.keyMoments && video.videoMetadata.keyMoments.length > 0
    );
    const videosWithSubtitles = videosWithMetadata.filter(video => 
        video.videoMetadata?.subtitles && video.videoMetadata.subtitles.length > 0
    );
    
    const totalVideoDurationMinutes = videosWithMetadata.reduce((sum, video) => 
        sum + (video.videoMetadata?.durationMinutes || 0), 0
    );
    ...
    const unitSummaries = course.units.map(unit => {
        ...
        const videoCount = unit.topics.reduce((sum, topic) => 
            sum + topic.contents.filter(content => content.contentKind === "Video").length, 0
        );
        const videoDurationMinutes = unit.topics.reduce((sum, topic) => 
            sum + topic.contents.reduce((contentSum, content) => 
                contentSum + (content.videoMetadata?.durationMinutes || 0), 0
            ), 0
        );
